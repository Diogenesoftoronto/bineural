...
2. **Adaptive Load Balancing**: Automatic, performance-based routing powered by real-time metrics (requires `enable_enterprise: true`)
...
1. **Pricing Data** (Primary source)
   - Downloaded from a remote URL (configurable, defaults to `https://raw.githubusercontent.com/maximhq/bifrost/main/pricing/datasheet.json`)
...
<Info>
  Adaptive Load Balancing is available when `enable_enterprise: true` is set in your `config.json`.
</Info>
