...
<Warning>
  ...
  - If you need high availability and enterprise capabilities such as real-time synchronization, enable `enable_enterprise: true` in your config.json.
...
<Warning>
Without P2P clustering, there's no mechanism to notify other nodes of configuration changes. For multinode deployments without clustering, use the shared `config.json` approach instead. Enable clustering with `enable_enterprise: true`.
</Warning>
